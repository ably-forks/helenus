...
var Connection = function(options){
  ...
  /**
   * TLS options to pass to tls.connect
   */
...
Connection.prototype.connect = function(callback){
  ...
  var stream;
  if(this.tls) {
	stream = tls.connect(this.port, this.host, this.tls);
	//since thrift only looks for the connect event, we synthesise one
	stream.on('secureConnect', function() { stream.emit('connect'); });
  } else {
	stream = net.createConnection(this.port, this.host);
  }
  this._connection = new thrift.Connection(stream);
